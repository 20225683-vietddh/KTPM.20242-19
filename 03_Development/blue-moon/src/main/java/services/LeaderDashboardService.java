...
public class LeaderDashboardService implements DashboardService {
    //private final HouseholdDAO householdDAO;
    //private final ResidentDAO residentDAO;
    private final CampaignFeeDAO campaignFeeDAO;
    private final FeeDAO feeDAO;

    public LeaderDashboardService() {
        try {
    ...
    public XYChart.Series<String, Number> getCampaignFeeHistory() {
        ...
        try {
            List<Map<String, Object>> campaigns = campaignFeeDAO.getLastNCampaignsWithStatus(6);
            for (Map<String, Object> campaign : campaigns) {
                series.getData().add(new XYChart.Data<>(
                    campaign.get("name").toString(),
                    ((Number) campaign.get("completion_rate")).doubleValue()
                ));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Map<String, Object>> getTypicalFees() {
        try {
            return feeDAO.getLastNFeesWithStatus(5);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<Map<String, Object>> getRecentResidentChanges() {
        //return residentDAO.getRecentChangesWithDetails(5);
